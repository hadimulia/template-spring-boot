    ...
    var lm = new ListManager({
        ...
        rowRenderer: function (row, index) {
            var deleteForm = '<form action="/roles/' + row.id + '/delete" method="post" class="d-inline">' +
                '<button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#confirmModal" data-modal-title="' + appI18n.confirmDeleteRoleTitle + '" data-modal-body="' + appI18n.confirmDeleteRoleBody + '" data-modal-btn="' + appI18n.modalConfirmDelete + '"><i class="bi bi-trash"></i></button></form>';
            return '<tr>' +
                '<td>' + index + '</td>' +
                '<td>' + (row.name || '') + '</td>' +
